...
nuisance parameter to correct for. The [occARU
...
occARU requires CmdStan \>= 2.36.0. If you have not used Stan before,
run:
...
accepts deployment and observation data frames following the
...
processes, Dirichlet variance decomposition, and Poisson observation
...
the log prior density (`lprior`), and posterior predictions for the full
detection history (`yrep`) or aggregated site-by-species counts
(`Qrep`). Optionally, Monte Carlo integration over site and
observation-level random effects produces marginal log likelihoods
(`log_lik2`) with improved PSIS-LOO-CV performance.
