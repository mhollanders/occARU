...
occARU automatically detects the deployment history of each site and sites sampled in more than one season are modelled with dynamic occupancy [@mackenzie2003] with a continuous time transition process (with subscripts for $i$, $k$, and $s$ dropped on $\boldsymbol{P}$ and $\boldsymbol{Q}$):
