...
When predictors are included alongside spatial or temporal random effects, the fixed and random effects can be confounded because the random effects absorb part of the variance that the fixed effects are trying to explain [@hodges2010]. In practice, fixed effect coefficients are often "lost" while the random effects capture most of the variance. A potential remedy is to compute the "unconditional" coefficients from the "conditional" coefficients (conditioned on the presence of the random effect) by adding back the variance absorbed by the random effect to the fixed effects, 
...
occARU accommodates groups of ARUs nested in regions, for example when multiple independent grids are deployed. When multiple regions are included, occARU includes region-level intercepts for both occupancy and detection. Random (spatial) site effects are also computed separately per-region, which can significantly increase speed of model fits.
